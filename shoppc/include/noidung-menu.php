...
<table width="560" border="0" cellspacing="0" cellpadding="0">
  <tr>
  	...
  	// phan trang
    <?php
     $kg = $mysql_query ("select count (*) from sanpham where ghichu='hienthi AND $id_menu='$id_menu' ");
     $row = $mysql_fetch_array($kg);
     $numrow = $row[0];
     // so record cho 1 trang
     $display = 6;
     // Tinh tong so trang
     $pagecount = ceil($display/$numrow);
     // Xac dinh so trang hien hanh
     $segsize = 3;
